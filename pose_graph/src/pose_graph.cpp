...
void PoseGraph::savePoseGraph() {
  ...
  for (it = keyframelist.begin(); it != keyframelist.end(); it++) {
    ...
    fprintf(pFile, " %d %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %d %f %f %f %f %f %f %f %f %d\n", (*it)->index, (*it)->time_stamp, VIO_tmp_T.x(),
            VIO_tmp_T.y(), VIO_tmp_T.z(), PG_tmp_T.x(), PG_tmp_T.y(), PG_tmp_T.z(), VIO_tmp_Q.w(), VIO_tmp_Q.x(), VIO_tmp_Q.y(), VIO_tmp_Q.z(),
            PG_tmp_Q.w(), PG_tmp_Q.x(), PG_tmp_Q.y(), PG_tmp_Q.z(), (*it)->loop_index, (*it)->loop_info(0), (*it)->loop_info(1), (*it)->loop_info(2),
            (*it)->loop_info(3), (*it)->loop_info(4), (*it)->loop_info(5), (*it)->loop_info(6), (*it)->loop_info(7), (int)(*it)->keypoints.size());
...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop) {
  ...
  int loop_index = flag_detect_loop ? detectLoop(cur_kf, cur_kf->index) : -1;
  if (!flag_detect_loop) {
  ...
  if (loop_index != -1) {
    ...
    if (cur_kf->findConnection(old_kf)) {
      ...
      Eigen::Vector3d w_P_old, vio_P_cur;
      Eigen::Matrix3d w_R_old, vio_R_cur;
      ...
      Eigen::Vector3d w_P_cur = w_R_old * relative_t + w_P_old;
      Eigen::Matrix3d w_R_cur = w_R_old * relative_q;
      ...
      Eigen::Vector3d shift_t = w_P_cur - w_R_cur * vio_R_cur.transpose() * vio_P_cur;
      // shift vio pose of whole sequence to the world frame
      if (old_kf->sequence != cur_kf->sequence && !sequence_loop[cur_kf->sequence]) {
        w_r_vio = shift_r;
        w_t_vio = shift_t;
        vio_P_cur = w_r_vio * vio_P_cur + w_t_vio;
        vio_R_cur = w_r_vio * vio_R_cur;
...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index) {
  ...
  if (DEBUG_IMAGE) {
    ...
    for (unsigned int i = 0; i < ret.size(); i++) {
      ...
      putText(tmp_image, "index:  " + to_string(tmp_index) + "loop score:" + to_string(ret[i].Score), cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX,
              0.5, cv::Scalar(255));
...
void PoseGraph::optimize4DoF() {
  while (true) {
    ...
    auto it = keyframelist.begin();
    for (int i = 0; it != keyframelist.end(); ++it, ++i) {
    ...
    for (int i = 0; it != keyframelist.end(); ++it, ++i) {
      ...
      Eigen::Quaterniond tmp_q{Utility::ypr2R(Eigen::Vector3d(euler_array[i][0], euler_array[i][1], euler_array[i][2]))};

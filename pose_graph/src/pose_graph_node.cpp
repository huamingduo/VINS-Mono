...
void image_callback(const sensor_msgs::ImageConstPtr &image_msg) {
  static double last_image_time = image_msg->header.stamp.toSec();
  // ROS_INFO("image_callback!");
  if (!LOOP_CLOSURE) {
    return;
  }
  // detect unstable camera stream
  if (image_msg->header.stamp.toSec() - last_image_time > 1.0 || image_msg->header.stamp.toSec() < last_image_time) {
...
void imu_forward_callback(const nav_msgs::Odometry::ConstPtr &forward_msg) {
  ...
  Eigen::Vector3d vio_t(forward_msg->pose.pose.position.x, forward_msg->pose.pose.position.y, forward_msg->pose.pose.position.z);
...
void relo_relative_pose_callback(const nav_msgs::Odometry::ConstPtr &pose_msg) {
  Eigen::Vector3d relative_t = Eigen::Vector3d(pose_msg->pose.pose.position.x, pose_msg->pose.pose.position.y, pose_msg->pose.pose.position.z);
  Eigen::Quaterniond relative_q = Eigen::Quaterniond(pose_msg->pose.pose.orientation.w, pose_msg->pose.pose.orientation.x,
...
void vio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg) {
  ...
  Eigen::Quaterniond vio_q = Eigen::Quaterniond(pose_msg->pose.pose.orientation.w, pose_msg->pose.pose.orientation.x,
                                                pose_msg->pose.pose.orientation.y, pose_msg->pose.pose.orientation.z);
...
void extrinsic_callback(const nav_msgs::Odometry::ConstPtr &pose_msg) {
  ...
  tic = Eigen::Vector3d(pose_msg->pose.pose.position.x, pose_msg->pose.pose.position.y, pose_msg->pose.pose.position.z);
  qic = Eigen::Quaterniond(pose_msg->pose.pose.orientation.w, pose_msg->pose.pose.orientation.x, pose_msg->pose.pose.orientation.y,
...
void process() {
  ...
  while (true) {
    ...
    // build keyframe
    Eigen::Vector3d T = Eigen::Vector3d(pose_msg->pose.pose.position.x, pose_msg->pose.pose.position.y, pose_msg->pose.pose.position.z);
    Eigen::Matrix3d R = Eigen::Quaterniond(pose_msg->pose.pose.orientation.w, pose_msg->pose.pose.orientation.x, pose_msg->pose.pose.orientation.y,
                                           pose_msg->pose.pose.orientation.z)
                            .toRotationMatrix();
    ...
    KeyFrame *keyframe =
        new KeyFrame(pose_msg->header.stamp.toSec(), frame_index, T, R, image, point_3d, point_2d_uv, point_2d_normal, point_id, sequence);
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber sub_imu_forward = n.subscribe("/vins_estimator/imu_propagate", 2, imu_forward_callback);
  ros::Subscriber sub_vio = n.subscribe("/vins_estimator/odometry", 2, vio_callback);
  ros::Subscriber sub_image = n.subscribe(IMAGE_TOPIC, 2, image_callback);
  ros::Subscriber sub_pose = n.subscribe("/vins_estimator/keyframe_pose", 2, pose_callback);
  ros::Subscriber sub_extrinsic = n.subscribe("/vins_estimator/extrinsic", 2, extrinsic_callback);
  ros::Subscriber sub_point = n.subscribe("/vins_estimator/keyframe_point", 2, point_callback);
  ros::Subscriber sub_relo_relative_pose = n.subscribe("/vins_estimator/relo_relative_pose", 2, relo_relative_pose_callback);

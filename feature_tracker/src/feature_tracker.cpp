...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time) {
  cur_time = _cur_time;
  cv::Mat img = EqualizeIfNeeded(_img, 3.0, cv::Size(8, 8));
...
cv::Mat FeatureTracker::EqualizeIfNeeded(const cv::Mat &image, const double &clip_limit, const cv::Size &tile_grid_size) const {
  if (!EQUALIZE) {
    return image;
  }
...
void FeatureTracker::setMask() {
  ...
  mask = FISHEYE ? fisheye_mask.clone() : cv::Mat(ROW, COL, CV_8UC1, cv::Scalar(255));
...
void FeatureTracker::undistortedPoints() {
  static map<int, cv::Point2f> prev_un_pts_map;
